...
#define switch1 13              //ปุ่มที่หนึ่ง
#define switch2 12              //ปุ่มที่สอง
#define switch3 11              //ปุ่มที่สาม
#define switch4 10              //ปุ่มที่สี่
#define buzzer 6                //buzzer
#define LDR_SENSOR A2           //ใช้วัดความสว่างของแสง
...
void setup()
{
  ...
  Timer1.attachInterrupt(count);                            //กำหนดฟังก์ชันเพื่อรองรับการอินเตอร์รัพ
  OLED.clearDisplay();
...
void countdown()
{
  ...
  if (digitalRead(switch3) == 0) //EEPROM
  {
    countdownstatus = 1;                                                          //start countdown
    delay(200);
  }
